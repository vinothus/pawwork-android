...
*Update after cold-restart attempts:* the emulator could not be revived on this host — `/dev/kvm` is absent, so x86_64 emulation fell back to TCG software mode, and both AVDs (`dev_phone`, `fresh_phone`) crashed during boot (QEMU TCG instability). Compounding it, the original emulator's WebView renderer was crash-looping (`FATAL:crashpad_client_linux.cc … not handled by all webviews` + SystemUI ANR) — a trivial `hello.html` control page also failed to render, proving the emulator, not the APK. `drone/template.apk` (850,985 B) is ready to side-load; on-device JS-render proof needs a KVM-enabled host or a real device.
